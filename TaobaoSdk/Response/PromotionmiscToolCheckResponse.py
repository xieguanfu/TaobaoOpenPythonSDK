...
## @brief UMP工具检测。ISV通过该接口检测（通过taobao.ump.tool.add）创建的UMP工具（tool）是否符合规范，如果不符合，则返回错误信息和对应的解决方案的；工具检测通过后才可以提交工具审核邮件，提交工具审核时，需提供该接口的返回值。
...
from Domain.CheckToolModule import CheckToolModule

        

## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: UMP工具检测。ISV通过该接口检测（通过taobao.ump.tool.add）创建的UMP工具（tool）是否符合规范，如果不符合，则返回错误信息和对应的解决方案的；工具检测通过后才可以提交工具审核邮件，提交工具审核时，需提供该接口的返回值。</SPAN>
# <UL>
# </UL>
class PromotionmiscToolCheckResponse(object):
    def __init__(self, kargs=dict()):
        ...
        # <UL>
        # <LI>
        # <SPAN style="color:DarkRed; font-size:18px; font-family:'Times New Roman',Georgia,Serif;">Type</SPAN>: <SPAN style="color:DarkMagenta; font-size:16px; font-family:'Times New Roman','宋体',Georgia,Serif;">CheckToolModule</SPAN>
        # </LI>
        # <LI>
        ...
        # </LI>
        # </UL>
        self.check_tool_module = None
        
        
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">接口调用错误信息描述。</SPAN>
        # <UL>
        # <LI>
        ...
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">工具检测动作是否成功。检测是否通过请查看返回值default_model里的is_pass。</SPAN>
        # <UL>
        # <LI>
    ...
    def _getPropertyType(self, name):
        properties = {
            ...
            "check_tool_module": "CheckToolModule",
    ...
    def __init(self, kargs):
        ...
        if kargs.has_key("check_tool_module"):
            self.check_tool_module = self._newInstance("check_tool_module", kargs["check_tool_module"])
