...
## @brief 业务状态为成功的时候，会发放店铺积分给买家,同时会卖家店铺积分冻结部分会变为以发放；业务状态为失败的时候，会扣解冻卖家发放的店铺积分，及状态由预扣的冻结状态改为可发放状态；
...
## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: 业务状态为成功的时候，会发放店铺积分给买家,同时会卖家店铺积分冻结部分会变为以发放；业务状态为失败的时候，会扣解冻卖家发放的店铺积分，及状态由预扣的冻结状态改为可发放状态；</SPAN>
# <UL>
# </UL>
class UppShoppointSendResponse(object):
