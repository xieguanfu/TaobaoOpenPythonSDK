...
from Domain.BatchPolicy import BatchPolicy

        

## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: 政策全量增加,会删除政策中字段source值为TOP的记录，并且把新增的记录插入，传入文件大小限制在5M(一般10w条记录不会超过5M),每半小时只允许调用一次</SPAN>
# <UL>
# </UL>
class JipiaoPoliciesFulladdResponse(object):
    def __init__(self, kargs=dict()):
        ...
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">此对象无效，只是空引用</SPAN>
        # <UL>
        # <LI>
        # <SPAN style="color:DarkRed; font-size:18px; font-family:'Times New Roman',Georgia,Serif;">Type</SPAN>: <SPAN style="color:DarkMagenta; font-size:16px; font-family:'Times New Roman','宋体',Georgia,Serif;">BatchPolicy</SPAN>
        # </LI>
        # <LI>
        ...
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">当前调用生成的编号，方便与异步返回的结果对应</SPAN>
        # <UL>
        # <LI>
        ...
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">机票政策上传结果，true，成功；false，失败</SPAN>
        # <UL>
        # <LI>
    ...
    def __init(self, kargs):
        ...
        if kargs.has_key("empty"):
            self.empty = self._newInstance("empty", kargs["empty"])
        
        if kargs.has_key("invoke_id"):
            self.invoke_id = self._newInstance("invoke_id", kargs["invoke_id"])
