...
## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: 提供御膳房任务状态查询</SPAN>
# <UL>
# </UL>
class ClouddataMbpStatusGetResponse(object):
    def __init__(self, kargs=dict()):
        ...
        ## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">状态查询的返回结果头信息。 当query_type为1的时候，有status,begin_time,end_time三个字段  status的状态如下:    1:等待 2：就绪 3:运行中 4:成功 5:失败 beginTime:任务开始运行时间，只有status为3、4、5时候有效,格式是yyyy-MM-dd HH:mm:ss，其他情况为空 endTime:任务结束时间，只有status为4、5时候有效,格式是yyyy-MM-dd HH:mm:ss，其他情况下为空</SPAN>
        # <UL>
        # <LI>
