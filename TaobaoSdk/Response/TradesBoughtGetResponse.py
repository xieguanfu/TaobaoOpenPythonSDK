...
## @brief <SPAN style="font-size:16px; font-family:'宋体','Times New Roman',Georgia,Serif;">Response: 搜索当前会话用户作为买家达成的交易记录(目前只能查询三个月以内的订单)</SPAN>
# <UL>
# <LI>
...
# </LI>
# </UL>
class TradesBoughtGetResponse(object):
    def __init__(self, kargs=dict()):
        ...
        ''' 
        @ivar total_results: 搜索到的交易信息总数; B{Level}: C{Basic}; B{Required}: C{true}; B{Sample}: C{123};
        ...
        self.trades = None
        ''' 
        @ivar trades: 搜索到的交易信息列表，返回的Trade和Order中包含的具体信息为入参fields请求的字段信息; B{Level}: C{Object Array}; B{Required}: C{true}; B{Sample}: C{};
        @type trades: Trade
        '''
